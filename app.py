...
st.header('Deciscions are hard, I got u bro. Welcome to "what would large imp do?"')
...
if age_st == '15-17' :
    ...
    if school_choice == 'no' :
        rod = st.subheader('do your fucking homework, punk. You can come back when that is done.')
if age_st != '15-17' :
...
with st.expander('have some random numbers that are mostly just for me') :
    st.write(high_craft)
...
if high_friend_game is top1  :
    ...
    st.video(video_file)
    st.write('video creds slimcicle on youtube')
if play_social is top1 :
    video_file = open('im_gaymer.mp4','rb')
    gaymer = video_file.read()
    st.video(video_file)
    st.write('video credits slimecicle on youtube')
img = st.image(vibe)
